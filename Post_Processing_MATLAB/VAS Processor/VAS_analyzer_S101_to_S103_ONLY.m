...
btn_num_types = ["VAS_filename_4BTN" "VAS_filename_10BTN"];

for sub_num = subj_num
    for btn_idx = 1:length(btn_num_types)
        
        btn_num_type = btn_num_types(btn_idx);
        xls_sheet = subject(sub_num).(btn_num_type);
        ...
        % load file & remove NaNs
        exp_data_sheet = readtable(xls_sheet);
        exp_data_sheet = rmmissing(exp_data_sheet,2); 
        sheet_rows = size(exp_data_sheet,1);

        exp_data_sheet = utils.set_Header_101_to_103_ONLY(exp_data_sheet);
        
        btn_num = exp_data_sheet.btn_option;
        trial_num = exp_data_sheet.trial;
        pres_num = exp_data_sheet.pres;
        max_trial_num = max(trial_num);
        max_pres_num = max(pres_num);
        total_options = max_pres_num*btn_num(1);

        torques = table2array(exp_data_sheet(:, contains(exp_data_sheet.Properties.VariableNames, 'torque')));
        values = table2array(exp_data_sheet(:, contains(exp_data_sheet.Properties.VariableNames, 'mv')));

        % reshape/compile torques & values into columns for each trial
        % i.e. each column is a distinct trial, with presentations in
        ...
        % save to compiled_data struct
        compiled_data.("S10" + sub_num).(btn_num_type) = struct('reshaped_torques', reshaped_torques, ...
...
%% Plot 4-button Raw Data of all subjects together
subj_num = [1 2 3];
...
for sub_num = subj_num
    ...
    if isfield(compiled_data.("S10" + sub_num), 'VAS_filename_4BTN')
        [reshaped_torques, reshaped_values, total_options, ...
                max_trial_num] = utils.unpack_4BTN_S101_to_S103_ONLY(compiled_data, sub_num);
    else
        continue
    end
...
colors = {"#0072BD", "#D95319", "#EDB120", "#A2142F", "#7E2F8E", "#4DBEEE"};
for sub_num = subj_num
    y10 = avg_traj_4btn_y(:,sub_num);
    plot(x_avg,y10,'color',colors{sub_num}, 'LineWidth',2);
    hold on
end
xlabel("Normalized Torque")
ylabel("Normalized $/Hour")
...
for sub_num = subj_num
    ...
    if isfield(compiled_data.("S10" + sub_num), 'VAS_filename_10BTN')
        [reshaped_torques, reshaped_values, total_options, ...
            max_trial_num] = utils.unpack_10BTN_S101_to_S103_ONLY(compiled_data, sub_num);
    else
        continue
    end
